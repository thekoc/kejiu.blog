...
draft: false
summary: '当上游服务器是 HTTPS 服务时，Nginx 反向代理需要做一些额外的配置。'
images: []
authors: ['default']
---
...
但是如果后端是 HTTPS 服务，看似可以直接把 `proxy_pass` 的值改为 `https://backend.example.com`，但是这样的修改存在隐患。一般而言，我们使用 HTTPS 服务，就是为了保证数据传输的安全性，但是 Nginx 默认并不校验后端的证书，这种情况下所谓的 HTTPS 连接完全是掩耳盗铃。
...
看似已经很完美了，但是 Nginx 很坑的一点是，默认 SNI (Server Name Indication) 并没有开启。SNI 是 TLS 协议的一个扩展。简单来说，当一个主机同时运行多个域名的 HTTPS 服务时，它并不知道客户端请求的是哪个域名，因此可能返回不正确的证书。SNI 就是为了解决这一点。在 Nginx 中这个对应着 `proxy_ssl_server_name` 选项。
